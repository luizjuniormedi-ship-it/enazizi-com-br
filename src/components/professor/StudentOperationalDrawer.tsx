 ...
 * Consome:
 *   - action `student_detail` (real, já existente) → cognitivo + atividade + simulados
 *   - tabela `assistant_decisions` (timeline de decisões IA, se RLS permitir)
 * Sem dado → DadosInsuficientesCard nas seções vazias.
 * Sem mocks. Sem KPIs inventados.
 */
...
import { Activity, Brain, Clock, AlertCircle, History, Target, Loader2 } from "lucide-react";
...
interface Props {
  ...
  onAssignRecovery?: (studentId: string, suggestedSpecialty?: string) => void;
      ...
      <SheetContent side="right" className="w-full sm:max-w-xl p-0 flex flex-col">
        <SheetHeader className="px-6 pt-6 pb-3 border-b border-white/5">
          <SheetTitle className="text-xl font-black">
            {detail?.profile?.display_name || "Aluno"}
          </SheetTitle>
        ...
        <div className="px-6 py-3 border-b border-white/5 flex flex-wrap gap-2">
          {onAssignRecovery && (
            <Button
              size="sm"
              onClick={() => studentId && onAssignRecovery(studentId, weakest?.specialty)}
              className="h-8 text-[11px] font-bold uppercase tracking-wider"
            >
              <Target className="h-3 w-3 mr-1.5" /> Atribuir Recovery
            </Button>
          )}
          {onOpenMentor && (
            <Button
              size="sm"
              variant="outline"
              onClick={() => studentId && onOpenMentor(studentId)}
              className="h-8 text-[11px] font-bold uppercase tracking-wider"
            >
              Mentoria
            </Button>
          ...
          <div className="p-6 space-y-6">
            ...
            {!loading && detail && (
              <>
                ...
                <Section icon={<Brain className="h-3.5 w-3.5" />} title="Cognitivo">
                  <KpiGrid
                    items={[
                      ...
                      {
                        label: "Acerto geral",
                        value: detail.study_performance
                          ? `${detail.study_performance.taxa_acerto}%`
                          : "—",
                      },
                      {
                        label: "Questões",
                        value: detail.study_performance?.questoes_respondidas ?? 0,
                      },
                      {
                        label: "Especialidades",
                        value: detail.domain_scores?.length ?? 0,
                      },
                    ]}
                  />
                  <p className="text-[10px] text-white/40 mt-2">
                    Theta/TRI · Stability FSRS · Recovery load: em integração — aparecerão aqui quando o pipeline cognitivo expor essas métricas por aluno.
                  </p>
                </Section>
                ...
                <Section icon={<AlertCircle className="h-3.5 w-3.5" />} title="Temas críticos">
                  {detail.error_topics?.length > 0 ? (
                    ...
                    <DadosInsuficientesCard
                      ...
                      description="O aluno ainda não acumulou erros suficientes para gerar lista de temas críticos."
                    />
                  )}
                </Section>

                {/* SIMULADOS / ATIVIDADES RECENTES */}
                ...
                {/* TIMELINE DE INTERVENÇÕES IA */}
                <Section icon={<History className="h-3.5 w-3.5" />} title="Decisões pedagógicas (IA)">
                  {decisions && decisions.length > 0 ? (
                    <div className="space-y-2">
                      {decisions.map((d) => (
                        ...
                        >
                          ...
                          <span className="text-[10px] text-white/30 mt-1 inline-block">
                            via {d.source_module}
                          </span>
                        </div>
                      ))}
                    ...
                    <DadosInsuficientesCard
                      title="Sem decisões pedagógicas registradas"
                      description="Nenhuma intervenção IA foi registrada para este aluno ainda."
                    />
                  )}
                </Section>
              </>
      ...
      {items.map((it, i) => (
        ...
        >
          ...
          <div className="flex items-center gap-2 shrink-0">
            {it.score != null && (
              <span className="text-xs font-black text-white">{it.score}%</span>
            )}
            <Badge
              variant="outline"
              className={cn(
