  ...
  plugins: [
    ...
    VitePWA({
      ...
      workbox: {
        ...
        runtimeCaching: [
          {
            // Always go to network for HTML navigations so new releases are picked up immediately
    ...
    rollupOptions: {
      output: {
        manualChunks(id) {
          ...
          if (/node_modules[\\/]react(?:-dom)?[\\/]|node_modules[\\/]react-router(?:-dom)?[\\/]/.test(id)) return "vendor-react";
