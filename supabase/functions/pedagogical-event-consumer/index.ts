...
Deno.serve(enterpriseEdgeHandler("pedagogical-event-consumer", async ({ req, logger, supabaseAdmin }) => {
  ...
  try {
    ...
    if (Object.keys(updates).length > 0) {
      await supabaseAdmin
        .from("cognitive_states")
        .update({ ...updates, metadata: { ...cogState.metadata, ...metadata }, last_event_id: eventId, updated_at: new Date().toISOString() })
        .eq("id", cogState.id);
    }
    ...
    logger.error("COG_RUNTIME_FAIL", err.message, { eventId });
    await supabaseAdmin.rpc("mark_pedagogical_event_consumed", {
      event_id: eventId,
      consumer_name: "cognitive-event-runtime-v2.5",
      success: false,
