...
REGRA: Gere SEMPRE entre 2 e 4 sub-perguntas separadas, cada uma focada em um aspecto diferente do caso.`;
        ...
        if (!response.ok) {
          logAiUsage({ userId: user.id, functionName: "discursive-questions", modelUsed: "openai/gpt-4o-mini", success: false, responseTimeMs: elapsed, cacheHit: false, modelTier: "fast" }).catch(() => {});
        ...
        logAiUsage({ userId: user.id, functionName: "discursive-questions", modelUsed: "openai/gpt-4o-mini", success: true, responseTimeMs: elapsed, cacheHit: false, modelTier: "fast" }).catch(() => {});
