...
if (!LOVABLE_API_KEY) {
  console.error("[question-generator] CRITICAL: LOVABLE_API_KEY missing");
}
      ...
      if (isGenericRequest) {
        ...
        if (lookup.hit && lookup.content?.questions?.length) {
          await logAIUsage({
            ...
            model: lookup.modelUsed || "google/gemini-2.5-flash", cacheStatus: "hit",
        ...
        await logAIUsage({
          ...
          model: "google/gemini-2.5-flash", cacheStatus: "bypass",
...
${prevSnapshot.length > 0 ? `\nNÃO REPITA:\n${prevSnapshot.slice(0, 40).map((s, i) => `${i + 1}. ${String(s).slice(0, 100)}`).join("\n")}` : ""}`;
          ...
          const runBatch = async (batchIdx: number, slotTarget?: any) => {
            ...
            try {
              const resp = await aiFetch({
                model: qualityProfile?.preferred_model || "google/gemini-2.5-flash",
                messages: [
                  { role: "system", content: systemPrompt }, 
                  ...
                  validatedBatch.push({
                    ...
                    medical_audit: { 
                      ...
                      adaptive_routing: {
                        ...
                        model_used: qualityProfile?.preferred_model || 'google/gemini-2.5-flash',
      ...
      if (qgCacheEligible && qgCacheHash && allQuestions.length === requestedCount && allQuestions.length > 0) {
        ...
        await saveAIResponseToCache({
          ...
          modelUsed: qualityProfile?.preferred_model || "google/gemini-2.5-flash",
    ...
    try {
      response = await aiFetch({ ...aiFetchOptions, model: "google/gemini-2.5-flash" });
    } catch (aiErr) {
