      ...
      if (isGenericRequest) {
        ...
        if (lookup.hit && lookup.content?.questions?.length) {
          await logAIUsage({
            ...
            model: lookup.modelUsed || "openai/gpt-5-mini", cacheStatus: "hit",
        ...
        await logAIUsage({
          ...
          model: "openai/gpt-5-mini", cacheStatus: "bypass",
...
${prevSnapshot.length > 0 ? `\nNÃO REPITA:\n${prevSnapshot.slice(0, 40).map((s, i) => `${i + 1}. ${String(s).slice(0, 100)}`).join("\n")}` : ""}`;
                  ...
                  validatedBatch.push({
                    ...
                    medical_audit: { 
                      ...
                      adaptive_routing: {
                        ...
                        model_used: qualityProfile?.preferred_model || 'openai/gpt-5-mini',
      ...
      if (qgCacheEligible && qgCacheHash && allQuestions.length === requestedCount && allQuestions.length > 0) {
        ...
        await saveAIResponseToCache({
          ...
          modelUsed: qualityProfile?.preferred_model || "openai/gpt-5-mini",
    ...
    try {
      response = await aiFetch({ ...aiFetchOptions, model: "openai/gpt-5-mini" });
    } catch (aiErr) {
