  ...
  try {
    ...
    if (action === "generate_case") {
      ...
      if (!response.ok) {
        ...
        logAiUsage({ userId: user.id, functionName: "practical-exam", modelUsed: "openai/gpt-4o-mini", success: false, responseTimeMs: elapsed, cacheHit: false, modelTier: "fast", errorMessage: `status ${response.status}` }).catch(() => {});
      ...
      logAiUsage({ userId: user.id, functionName: "practical-exam", modelUsed: "openai/gpt-4o-mini", success: true, responseTimeMs: elapsed, cacheHit: false, modelTier: "fast" }).catch(() => {});
    ...
    if (action === "evaluate") {
      ...
      if (!response.ok) {
        logAiUsage({ userId: user.id, functionName: "practical-exam-evaluate", modelUsed: "openai/gpt-4o-mini", success: false, responseTimeMs: elapsed2, cacheHit: false, modelTier: "fast" }).catch(() => {});
        return new Response(JSON.stringify({ error: "Erro ao avaliar desempenho" }), {
          status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
      logAiUsage({ userId: user.id, functionName: "practical-exam-evaluate", modelUsed: "openai/gpt-4o-mini", success: true, responseTimeMs: elapsed2, cacheHit: false, modelTier: "fast" }).catch(() => {});
