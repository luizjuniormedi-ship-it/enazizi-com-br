...
serve(async (req) => {
  ...
  try {
    ...
    let approved = false;
    const agentLogs: Array<{ agent: string; attempt: number; status: string; details: string }> = [];
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
      ...
      if (!genResult.ok) {
        agentLogs.push({ agent: "generator", attempt, status: "error", details: `HTTP ${genResult.status}` });
      ...
      if (!generated || !generated.items_map) {
        agentLogs.push({ agent: "generator", attempt, status: "parse_error", details: "JSON inválido" });
      ...
      if (!Array.isArray(generated.items_map) || generated.items_map.length !== cleanTerms.length) {
        agentLogs.push({ agent: "generator", attempt, status: "validation_error", details: `items_map length mismatch: ${generated.items_map?.length} vs ${cleanTerms.length}` });
      ...
      agentLogs.push({ agent: "generator", attempt, status: "ok", details: `Sigla: ${generated.sigla}` });
      ...
      if (!medResult.ok || !pedResult.ok) {
        agentLogs.push({ agent: "auditors", attempt, status: "error", details: "Audit call failed" });
        break;
      }
      ...
      if (!medAudit || !pedAudit) {
        agentLogs.push({ agent: "auditors", attempt, status: "parse_error", details: "Audit JSON parse failed" });
        break;
      }
      ...
      agentLogs.push({ agent: "medical_auditor", attempt, status: lastMedical.approved ? "approved" : "rejected", details: `Score: ${lastMedical.score} — ${lastMedical.summary}` });
      agentLogs.push({ agent: "pedagogical_auditor", attempt, status: lastPedagogical.approved ? "approved" : "rejected", details: `Score: ${lastPedagogical.score} — ${lastPedagogical.summary}` });
    ...
    if (!lastGenerated) {
      return json({ success: false, error: "Falha na geração após múltiplas tentativas.", agent_logs: agentLogs }, 500);
    }
    ...
    if (lastGenerated.prompt_imagem) {
      try {
        ...
        if (imgResp.ok) {
          ...
          agentLogs.push({ agent: "image_generator", attempt: 1, status: image_url ? "ok" : "no_image", details: image_url ? "Image generated" : "No image in response" });
        } else {
          agentLogs.push({ agent: "image_generator", attempt: 1, status: "error", details: `HTTP ${imgResp.status}` });
        }
      } catch (e) {
        agentLogs.push({ agent: "image_generator", attempt: 1, status: "error", details: String(e) });
      }
    }

    agentLogs.push({ agent: "consolidator", attempt: 1, status: "ok", details: `Final score: ${finalScore}, approved: ${approved}` });

    return json({
      success: approved || medScore >= 70,
      data: {
        tema: tema.trim(),
    ...
    console.error("generate-medical-mnemonic error:", err);
    return json(
