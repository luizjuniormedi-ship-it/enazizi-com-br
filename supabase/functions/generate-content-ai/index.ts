    ...
    if (!isRetry) {
      ...
      if (content.content_hash) {
        ...
        if (hashMatch) {
          ...
          await logPromptExecution(supabaseClient, contentId, content, 'openai/gpt-4o-mini', 0, 0, 0, Date.now() - startTime, 'valid', cacheStatus);
...
${parsedData.notebooklm_package?.audio_script || parsedData.video_script}
    ...
    if (contentId) {
      ...
      await logPromptExecution(supabaseClient, contentId, null, 'openai/gpt-4o-mini', 0, 0, 0, Date.now() - startTime, 'failed', 'cache_miss', null, null, error.message);
    }
