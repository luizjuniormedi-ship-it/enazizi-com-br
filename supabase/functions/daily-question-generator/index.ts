...
1. MÍNIMO 450 caracteres no enunciado (Padrão Ouro)
2. 4 alternativas (A, B, C, D), apenas 1 correta — TODAS clinicamente plausíveis
...
FORMATO JSON OBRIGATÓRIO (sem markdown):
{
  ...
  try {
    ...
    if (!response.ok) {
      ...
      logAiUsage({ userId: "system", functionName: "daily-question-generator-exam", modelUsed: "openai/gpt-5-mini-mini", success: false, responseTimeMs: elapsed, cacheHit: false, modelTier: "fast" }).catch(() => {});
      return 0;
    }
    logAiUsage({ userId: "system", functionName: "daily-question-generator-exam", modelUsed: "openai/gpt-5-mini-mini", success: true, responseTimeMs: elapsed, cacheHit: false, modelTier: "fast" }).catch(() => {});
...
FORMATO JSON OBRIGATÓRIO (sem markdown):
{
  ...
  try {
    ...
    if (!response.ok) {
      ...
      logAiUsage({ userId: "system", functionName: "daily-question-generator", modelUsed: "openai/gpt-5-mini-mini", success: false, responseTimeMs: elapsed, cacheHit: false, modelTier: "fast" }).catch(() => {});
      return 0;
    }
    logAiUsage({ userId: "system", functionName: "daily-question-generator", modelUsed: "openai/gpt-5-mini-mini", success: true, responseTimeMs: elapsed, cacheHit: false, modelTier: "fast" }).catch(() => {});
