...
BEGIN
  ...
  IF NEW.bank_question_id IS NOT NULL THEN
    ...
    IF v_exists THEN
      v_hash := 'sim:' || COALESCE(NEW.simulado_session_id::text, NEW.id::text) || ':' || COALESCE(NEW.question_index::text,'x');
  ...
  IF COALESCE(NEW.is_correct, true) = false THEN
    BEGIN
      INSERT INTO public.error_bank (user_id, tema, subtema, question_id, vezes_errado, dominado, created_at, updated_at)
      VALUES (NEW.user_id, v_topic, NEW.subtopic, NEW.bank_question_id, 1, false, now(), now())
      ON CONFLICT (user_id, tema_norm, subtema_norm, COALESCE(dominado, false))
      DO UPDATE SET vezes_errado = public.error_bank.vezes_errado + 1, updated_at = now();
