-- Add indexes for performance reporting if not already present
CREATE INDEX IF NOT EXISTS idx_telemetry_events_event_name_timestamp ON public.telemetry_events (event_name, timestamp);
CREATE INDEX IF NOT EXISTS idx_error_log_severity_created_at ON public.error_log (severity, created_at);
CREATE INDEX IF NOT EXISTS idx_ai_usage_logs_created_at ON public.ai_usage_logs (created_at);

-- RPC for the global optimization report
