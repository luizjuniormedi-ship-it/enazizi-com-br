-- Fix create_adaptive_decision RPC
...
DECLARE
    ...
    v_hash TEXT;
BEGIN
    -- Create a unique hash for idempotency if needed, using current column names
...
-- Fix get_unified_dashboard_data RPC
CREATE OR REPLACE FUNCTION public.get_unified_dashboard_data(
    p_user_id uuid, 
    p_reset_at timestamp with time zone, 
    p_today_iso timestamp with time zone
)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
    -- Basic counts - using fsrs_cards instead of non-existent flashcards
    SELECT count(*) INTO v_flashcards_count FROM fsrs_cards WHERE user_id = p_user_id;
    SELECT count(*) INTO v_uploads_count FROM uploads WHERE user_id = p_user_id;

    -- Accuracy calculation from practice_attempts
    ...
    -- Retention (FSRS) - Calculate retrievability based on stability and elapsed days
    -- Formula: R = 0.9 ^ (elapsed_days / stability)
    SELECT avg(pow(0.9, COALESCE(elapsed_days, 0) / NULLIF(stability, 0))) 
    INTO v_avg_retention 
    FROM fsrs_cards 
    WHERE user_id = p_user_id AND stability > 0;
    
    v_avg_retention := COALESCE(v_avg_retention, 0.9);
